...
import { Search, HandCoins, CircleAlert, ShoppingBag, TrendingUp, CircleCheck, CirclePlus } from "lucide-react";
...
export default function Page() {
    ...
    return (
        <>
            <CommonLayout>
                <section className="bg-(--grey2) rounded-[24px] py-2 px-5">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="relative">
                            ...
                            <Input placeholder='Search' className='rounded-[16px] w-109 h-14 pr-13 ' />
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-[1fr_438px] gap-4 mt-3">
                        <div className='grid grid-cols-1 md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-4 gap-4'>
                            {
                                dashboardData.map((item, index) => (
                                    <div key={index} className="bg-white rounded-[24px] py-3 px-5">
                                        <div className='flex items-center justify-between'>
                                            <h1 className='text-(--dark1) text-[20px] font-normal'>{item.title}</h1>
                                            <div style={{ backgroundColor: item.color }} className={`size-10 rounded-full flex items-center justify-center `}>
                                                {item.icon}
                                            </div>
                                        </div>
                                        <div className="mt-6 flex justify-between ">
                                                ...
                                                <h2 className="text-(--dark1) text-[40px] font-normal">
                                                    {item.value.includes('/') ? (
                                                        <>
                                                            ...
                                                            <span className="text-(--grey3)">/{item.value.split('/')[1]}</span>
                                                        </>
                                                    ) : (
                                                        item.value
                                                    )}
                                            ...
                                            <div className="flex items-end gap-1 h-20 w-25">
                                                {[40, 70, 50, 90, 60, 45].map((height, i) => (
                                                    <div
                                                        key={i}
                                                        className={`w-full rounded-full opacity-30 ${i === 3 ? 'opacity-100' : ''}`}
                                                        style={{
                                                            height: `${height}%`,
                                                            backgroundColor: item.title === 'Total Income' ? '#21A6E8' :
                                                                item.title === 'Total Orders' ? '#F5880B' :
                                                                    item.title === 'Order Due' ? '#EC3235' : '#0AAE78'
                                                        }}
                                                    ></div>
                                                ))}
                                            </div>
                        ...
                        <div className="bg-white rounded-[32px] p-4 border-2 border-[#FFFFFF80]">
                            <h1 className='text-(--dark1) text-[32px] font-normal'>Place your upcoming order</h1>
                            <p className='text-(--dark1) text-[14px] font-normal'>Create a new order or reorder items from your previous deliveries.</p>
                            <Button className='w-max mt-5 h-14.5 px-5 text-[20px] font-normal'>
                                <span>
